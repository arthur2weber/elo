...
export const triggerDriverGeneration = async (payload: DiscoveryPayload) => {
    ...
    try {
        ...
        const existing = list.find(d => d.ip === trackingKey);
        if (existing) {
        ...
        console.warn('Error checking existing devices:', e);
    }
    ...
    try {
        const targetIp = payload.ip || payload.addresses?.[0];
        let extraContext = {};
        ...
        while (attempt < maxAttempts) {
            ...
            const rawResponse = await runGeminiPrompt(prompts.generateDriver({
                ...
                rawInfo: JSON.stringify({ ...payload, scannedPorts: extraContext }, null, 2),
            ...
            if (verification.success) {
                ...
                await addDevice({
                    ...
                    ip: payload.ip || payload.addresses?.[0]
                });
                ...
                console.warn(`[DriverGenerator] Verification failed for ${safeDeviceName}: ${verification.error}`);
        ...
        console.error(`[DriverGenerator] Failed to generate driver for ${deviceKey}:`, error);
    } finally {
